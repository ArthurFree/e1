...
- 能力矩阵见下表（`src/platform/desktop/desktopCapabilities.ts`）：`localDirectory`、`documentPersistence`、`persistentAssetPaths`、`fileWatching`、`revealInFileManager`、`nativeSecrets` 为 true；`nativeMenu` 仍为 false（未实现，不得写「未来全 true」）。`nativeSecrets` 语义见表下注 2（R8-02：集成存在恒 true，本机实际安全后端由 `AppServices.secretStorageStatus` 运行时表达）。
...
² `nativeSecrets`（R008 Stage 1，R8-02）：Desktop 的 native secret 集成已真实存在（`DesktopSecretStore` → Main safeStorage），故「集成存在」恒为 true；本机**实际**安全后端（secure-persistent / session-only / unavailable）由运行时探测的 `secret.status` → `AppServices.secretStorageStatus` 表达（设置页降级提示的数据源），不走静态能力字段。语义与持久性由 `createDesktopRuntime.test.ts` 与 `electron/main/secrets/` 测试锁定。
