...
- 现有行为由 `src/editor/markdown.test.ts` 锁定：标题/段落/行内格式、任务列表、代码块、表格可解析往返；`localImage` 与 `attachment` 不序列化。
...
| `localImage`                                 | LocalImage（R004 阶段 6）         | 不产生（`![](...)` 解析为 image） | **不序列化，节点被丢弃**（测试锁定）             | **序列化为相对路径图片 `![alt](../assets/image.png)` + 二进制写入 `assets/`**；宽度和 alt 保留    | 附件缺失时占位并计入导入报告「缺失附件」      |
| `attachment`                                 | Attachment                        | 不产生                            | **不序列化，节点被丢弃**（与 localImage 同策略） | 序列化为相对路径链接 `[name](../assets/file.pdf)` + 二进制写入 `assets/`                          | 同上                                          |
...
**现状（R004 阶段 6 决策）**：`localImage` 与 `attachment` 在 Markdown 导出时**完全不序列化——节点被静默丢弃**（`markdown.test.ts` 锁定：输出不含 `![` 与 `attachment:`）。当初的理由是避免导出 `![](attachment:id)` 这种重新导入后无法渲染的伪引用。代价是：**当前 Markdown 导出不是完整可迁移备份**，正文中的图片与附件在导出物中消失。

**阶段 4 目标**：持久化级 MarkdownCodec 的 portable 模式必须做到——
