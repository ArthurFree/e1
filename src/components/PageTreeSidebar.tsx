...
interface PageTreeBodyProps {
  ...
  /**
   * 操作支持矩阵 page 组（R008 Stage 0，R8-01：AppServices.operations.page）：
   * 按页面 kind 取 document/group 子组门控——false 的动作隐藏入口
...
const PageTreeBody = memo(function PageTreeBody({
  ...
  // R008 Stage 0（R8-01）：行内动作按页面 kind 取 operation 子组——
  ...
  const onTreeKeyDown = (event: React.KeyboardEvent) => {
    ...
    if (event.key === "ArrowDown") {
      ...
      event.preventDefault();
      if (!collapsed.has(page.id)) toggleCollapse(page.id);
    } else if (event.key === "F2" && page && canRenamePage(page)) {
      event.preventDefault();
      startRename(page);
    }
  };
          ...
          {operations.page.group.create && (
            <button
              ...
              onClick={() => {
                ...
                // R008 Stage 0（§7.3）：group.rename 不支持的平台不得自动
                // 进入一个必然失败的重命名流程。
                void (async () => {
