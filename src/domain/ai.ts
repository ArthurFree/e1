...
export function buildPrompt(
  ...
  const systemByMode: Record<AIMode, string> = {
    ...
    polish:
      "你是一个文字润色助手。请润色用户提供的文字，保持原意不变，只改进表达与流畅度。请使用简体中文，以 Markdown 格式返回，只返回润色后的内容本身，不要附加解释。",
    rewrite:
      "你是一个文字改写助手。请改写用户提供的文字，保持核心信息不变，可以调整结构与措辞。请使用简体中文，以 Markdown 格式返回，只返回改写后的内容本身，不要附加解释。",
    summarize:
      "你是一个内容总结助手。请总结用户提供的选区或文档内容，提炼要点。请使用简体中文，以 Markdown 格式返回，只返回总结内容本身，不要附加解释。",
    draft:
      "你是一个中文写作助手。请围绕用户给出的主题撰写一篇结构完整、内容具体的文档；如果指定了文档类型，遵循该类型的常见结构。请使用简体中文，以 Markdown 格式返回，只返回文档正文本身，不要附加解释。",
  };
