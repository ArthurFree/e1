...
function makeWorkspace(
  partial: Partial<Workspace> & { id: string },
): Workspace {
  return {
    name: partial.id,
    icon: null,
...
describe("belongingPath", () => {
  it("拼接知识库与祖先分组链", () => {
    ...
    expect(belongingPath(doc, byId, "我的知识库")).toBe(
      "我的知识库 / 产品 / 发布会",
    );
    expect(belongingPath(makePage({ id: "root" }), byId, "我的知识库")).toBe(
      "我的知识库",
    );
  });
...
describe("buildActivityRows", () => {
  const workspaces = [
    makeWorkspace({ id: "ws1", name: "库一" }),
    makeWorkspace({ id: "ws2", name: "库二" }),
  ];
  const pages = [
...
describe("收藏排序", () => {
  it("favoriteWorkspaces 按收藏时间倒序", () => {
    ...
    expect(favoriteWorkspaces(workspaces).map((w) => w.id)).toEqual([
      "c",
      "d",
      "a",
    ]);
