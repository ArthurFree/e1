...
export type IpcErrorCode =
  ...
  /** R014：所选目录 vaultId 与最近记录不一致。 */
  | "VAULT_ID_MISMATCH"
  /** R014：跨库/搬迁预检过期。 */
  | "VAULT_TRANSFER_STALE_PLAN"
  /** R014：存在 dirty / 未保存文档，禁止转移。 */
  | "VAULT_TRANSFER_BLOCKED_DIRTY"
  /** R014：跨卷复制校验失败或部分完成。 */
  | "VAULT_TRANSFER_PARTIAL_FAILURE"
  /** R014：恢复跨库 Move 源会形成双 stable id。 */
  | "VAULT_TRANSFER_DUPLICATE_IDENTITY"
  /** R014.1：目标库已存在相同 Stable Note ID。 */
  | "VAULT_TRANSFER_IDENTITY_COLLISION"
  /** R014.1：目标库已存在相同 revision series。 */
...
const DOMAIN_TO_IPC: Record<string, IpcErrorCode> = {
  ...
  VAULT_TRANSFER_STALE_PLAN: "VAULT_TRANSFER_STALE_PLAN",
  VAULT_TRANSFER_BLOCKED_DIRTY: "VAULT_TRANSFER_BLOCKED_DIRTY",
  VAULT_TRANSFER_PARTIAL_FAILURE: "VAULT_TRANSFER_PARTIAL_FAILURE",
  VAULT_TRANSFER_DUPLICATE_IDENTITY: "VAULT_TRANSFER_DUPLICATE_IDENTITY",
  VAULT_TRANSFER_IDENTITY_COLLISION: "VAULT_TRANSFER_IDENTITY_COLLISION",
  VAULT_TRANSFER_REVISION_COLLISION: "VAULT_TRANSFER_REVISION_COLLISION",
};
...
const IPC_TO_DOMAIN: Partial<Record<IpcErrorCode, string>> = {
  ...
  VAULT_TRANSFER_STALE_PLAN: "DOCUMENT_CONFLICT",
  VAULT_TRANSFER_BLOCKED_DIRTY: "INVALID_INPUT",
  VAULT_TRANSFER_PARTIAL_FAILURE: "INVALID_INPUT",
  VAULT_TRANSFER_DUPLICATE_IDENTITY: "INVALID_INPUT",
  VAULT_TRANSFER_IDENTITY_COLLISION: "INVALID_INPUT",
  VAULT_TRANSFER_REVISION_COLLISION: "INVALID_INPUT",
};
