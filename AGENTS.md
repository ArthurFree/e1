...
质量：287 单元/组件测试、43 Playwright（功能 15、视觉基线 13 + 四档宽度 8 + 深色开始页、响应式 6）、typecheck、生产构建全部通过。（3 项文档编辑区视觉基线曾因环境渲染漂移失败，已用 git stash 验证与代码改动无关，并于 R003 第一批收尾时重新生成基线。）常用命令：`npm run dev`、`npm run build`、`npm test`、`npm run typecheck`、`npm run test:e2e`（截图基线更新用 `npm run test:e2e:update`）。
...
- `src/application/`：应用服务层——`services/SaveCoordinator.ts`（每文档保存串行队列 + 代次管理，附件清理/间隔版本只跟随最新快照）、`services/documentRecovery.ts`（localStorage 恢复缓冲，正文 JSON 经白名单校验）、`services/corruptedDiagnostics.ts`（损坏正文诊断记录）、`services/WorkspaceSessionService.ts`（知识库会话原子加载）、`services/PreferencesService.ts`（偏好写入串行队列 + 侧栏防抖 + 路由 last-write-wins）；仓储经构造函数注入（domain port），不直接依赖 IndexedDB 实现。
