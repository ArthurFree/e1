...
- `src/application/`：应用服务层——`AppServices.ts`（服务容器接口：8 个仓储 port + documentCommit + 会话服务 + 搜索索引 + 同步频道/存储事件 + AI/保存协调器工厂）、`services/SaveCoordinator.ts`（每文档保存串行队列 + 代次管理 + 快照 capturedAt + knownVersion 乐观锁，isCurrent 逐 await 重查，维护任务只跟随当前代次、维护失败经 onMaintenanceError 分流，正文提交走 DocumentContentCommitter 窄接口，冲突进 errorKind "conflict" 不自动重试）、`services/DocumentCommitService.ts`（R004：正文提交单点——commit/createWithContent/replaceContent/restoreRevision 统一落盘 + 搜索索引同步 + 同步广播）、`services/SearchIndexService.ts`（工作区级内存搜索索引）、`services/SyncChannelService.ts`（BroadcastChannel 多标签页同步 + tabId 回声抑制，R004 阶段 7）、`services/StorageQuotaService.ts`（存储估算与阈值，降级 null）、`services/StorageConnectionEventBus.ts`（db 连接事件到 UI 提示条的中转）、`services/documentRecovery.ts`（localStorage 恢复缓冲，正文 JSON 经白名单校验）、`services/corruptedDiagnostics.ts`（损坏正文诊断记录）、`services/WorkspaceSessionService.ts`（知识库会话原子加载：页面/标签/关联/正文，v4 起正文走工作区索引）、`services/PreferencesService.ts`（偏好写入串行队列 + 侧栏防抖 + 路由 last-write-wins + dispose/resume 生命周期）、`devDiagnostics.ts`（开发诊断指标，仅 dev 启用）；仓储经构造函数注入（domain port），不直接依赖 IndexedDB 实现。
- `src/state/`：`AppServicesProvider.tsx`（AppServices 容器注入，组件/状态层经 `useAppServices()` 取服务，禁止直接 import infrastructure）、`AppProviders.tsx`（组合层：Preferences → Workspace → Navigation → Overlay 四状态域 Provider 嵌套装配，R004 阶段 4）、`PreferencesProvider.tsx`（preferences + PreferencesService 实例，卸载时 dispose；内部路由通道 persistRoute/routePersistenceStatus/whenLoaded）、`WorkspaceProvider.tsx`（workspaces/会话/页面与标签 CRUD/搜索索引构建/初始加载；内部通道 loadSession/loadPages/getSnapshot）、`NavigationProvider.tsx`（view/selectedPageId/titleFocusPageId + 导航动作；经 `navigationBridge` 命令桥向工作区域暴露导航命令）、`workspace/sessionReducer.ts`（会话纯 reducer）、`WorkspaceSessionContext.tsx` / `NavigationContext.tsx` / `PreferencesContext.tsx` / `OverlayContext.tsx`（四个状态域窄 Context，value 形状不变）、`legacy/useApp.ts`（兼容聚合门面，仅供既有测试过渡）、`AppState.tsx`（兼容 re-export 层）。生产代码一律用窄 hook（useWorkspaceSession/useNavigation/usePreferences/useOverlay）。
...
已有命令：`npm run dev`（开发）、`npm run build`（类型检查 + 生产构建）、`npm test`（Vitest，jsdom + fake-indexeddb）、`npm run typecheck`、`npm run lint`（ESLint，0 error 门禁）、`npm run format:check` / `npm run format`（Prettier）、`npm run deps:check`（dependency-cruiser 分层与循环依赖）、`npm run test:coverage`、`npm run ci`（统一门禁：typecheck + lint + deps:check + test + build）、`npm run test:e2e` / `npm run test:e2e:update`。测试体系现状：
