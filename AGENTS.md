...
**R003（架构整改）第一批（阶段 0–3）已完成：并发/竞态测试基线（保存乱序、文档切换挂起保存、附件清理竞态、工作区连切、偏好并发更新）、`SaveCoordinator` 保存系统（每文档串行队列 + 代次管理，附件清理与间隔版本只跟随最新快照，localStorage 恢复缓冲 + 启动恢复提示条）、知识库会话原子化（`WorkspaceSessionService` 一次加载 + requestId 丢弃过期响应 + 单次 dispatch 提交）、偏好事务化（仓储 update 单事务 + `PreferencesService` 串行队列，侧栏宽度 250ms 防抖、路由 last-write-wins）。** 后续批次（应用层/DI、AppState 拆分、IndexedDB v3、正文校验）未做，见 `docs/requirements/R003.md`。

质量：244 单元/组件测试（含竞态基线与 1000 树节点、500 活动记录性能冒烟）、43 Playwright（功能 15、视觉基线 13 + 四档宽度 8 + 深色开始页、响应式 6）、typecheck、生产构建全部通过。（3 项文档编辑区视觉基线曾因环境渲染漂移失败，已用 git stash 验证与代码改动无关，并于 R003 第一批收尾时重新生成基线。）常用命令：`npm run dev`、`npm run build`、`npm test`、`npm run typecheck`、`npm run test:e2e`（截图基线更新用 `npm run test:e2e:update`）。
...
- `src/application/`：应用服务层——`services/SaveCoordinator.ts`（每文档保存串行队列 + 代次管理，附件清理/间隔版本只跟随最新快照）、`services/documentRecovery.ts`（localStorage 恢复缓冲）、`services/WorkspaceSessionService.ts`（知识库会话原子加载）、`services/PreferencesService.ts`（偏好写入串行队列 + 侧栏防抖 + 路由 last-write-wins）；仓储经构造函数注入（domain port），不直接依赖 IndexedDB 实现。
- `src/state/AppState.tsx`：应用状态 Provider（知识库会话 useReducer + requestId 原子加载、视图路由 `view`、页面/标签/回收站/搜索/收藏/最近浏览 action、AI 配置与设置面板开关），UI 只通过它和仓储接口取数。
...
- `src/test/`：Vitest 共享底座——`setup.ts`（jest-dom + fake-indexeddb）、`fixtures.ts`（确定性数据生成器 + deferred/sleep 时序工具）。
