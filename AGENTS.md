...
**R003 第三批（阶段 5，应用层与依赖注入）已完成：`AppServices` 服务容器 + `AppServicesProvider` 注入（生产 `createBrowserAppServices`，main.tsx 装配），12 个生产文件全部迁离 infrastructure 直接导入（Tiptap 附件扩展经 `editor.storage` 通道取仓储），`infrastructure/memory/` 内存仓储实现全部 7 个 port（复用 domain/pageTree，与 IndexedDB 版共用不变量断言），AppProvider 全流程内存容器集成测试，分层约束由 `src/test/architecture.test.ts`（import.meta.glob 源码扫描，替代 ESLint）强制。** 用例编排由 AppState actions + application 服务承载，不另建 use-case 类（决策见 decisions.md）。剩余批次：阶段 6（AppState 拆分）、阶段 7（IndexedDB v3 性能）、阶段 8（文档/ADR）。

质量：297 单元/组件测试、43 Playwright（功能 15、视觉基线 13 + 四档宽度 8 + 深色开始页、响应式 6）、typecheck、生产构建全部通过。（3 项文档编辑区视觉基线曾因环境渲染漂移失败，已用 git stash 验证与代码改动无关，并于 R003 第一批收尾时重新生成基线。）常用命令：`npm run dev`、`npm run build`、`npm test`、`npm run typecheck`、`npm run test:e2e`（截图基线更新用 `npm run test:e2e:update`）。

## 源码结构

- `src/domain/`：实体类型（`types.ts`）、页面树纯逻辑（`pageTree.ts`）、全局搜索（`search.ts`）、AI（`ai.ts`）、活动列表排序与归属路径（`activity.ts`）、创建位置选择（`picker.ts`）、路由持久化（`route.ts`）、版本策略（`revisions.ts`）、字数统计（`wordCount.ts`）、仓储接口（`repositories.ts`）、领域错误码（`errors.ts`，DomainError + 稳定 code）、正文 JSON 白名单校验与修复（`validation/documentContent.ts`，白名单与 extensions.ts schema 同步由测试强制）。
- `src/infrastructure/`：IndexedDB 实现——`db.ts`（DB v2 schema 与按 oldVersion 分支迁移，导出 `createV1Schema` 供迁移 fixture）、`repositories.ts`（含损坏数据降级、软删/恢复、版本与附件仓储、purge 级联、R003 阶段 4 关系约束）、`seed.ts`（预置知识库，模块级 Promise 防并发重复种子）、`aiProvider.ts`、`browserServices.ts`（生产服务装配根 `createBrowserAppServices`）、`memory/`（内存仓储：7 个 port 的纯内存实现 + `createInMemoryAppServices`，可替换性证明）、`migration.test.ts`（v1 真实 fixture 迁移测试）。
...
- `src/application/`：应用服务层——`AppServices.ts`（服务容器接口：7 个仓储 port + 会话服务 + AI/保存协调器工厂）、`services/SaveCoordinator.ts`（每文档保存串行队列 + 代次管理，附件清理/间隔版本只跟随最新快照）、`services/documentRecovery.ts`（localStorage 恢复缓冲，正文 JSON 经白名单校验）、`services/corruptedDiagnostics.ts`（损坏正文诊断记录）、`services/WorkspaceSessionService.ts`（知识库会话原子加载）、`services/PreferencesService.ts`（偏好写入串行队列 + 侧栏防抖 + 路由 last-write-wins）；仓储经构造函数注入（domain port），不直接依赖 IndexedDB 实现。
- `src/state/`：`AppServicesProvider.tsx`（AppServices 容器注入，组件/状态层经 `useAppServices()` 取服务，禁止直接 import infrastructure）、`AppState.tsx`（应用状态 Provider：知识库会话 useReducer + requestId 原子加载、视图路由 `view`、页面/标签/回收站/搜索/收藏/最近浏览 action、AI 配置与设置面板开关）。
...
- `src/test/`：Vitest 共享底座——`setup.ts`（jest-dom + fake-indexeddb + jsdom getClientRects polyfill）、`fixtures.ts`（确定性数据生成器 + deferred/sleep 时序工具）、`TestApp.tsx`（测试装配：IndexedDB 容器 + AppProvider）、`architecture.test.ts`（分层约束源码扫描：components/state/editor/application/domain 不得 import infrastructure，domain 不得 import react）。
