...
质量：437 单元/组件测试（含性能基准、v1–v4 迁移、两实现契约套件、多标签页同步、Context 渲染粒度）、47 Playwright（功能 15 + R004 写入路径 3 + 多标签页 1、视觉基线 13 + 四档宽度 8 + 深色开始页、响应式 6）、typecheck、lint、deps:check、生产构建全部通过。（3 项文档编辑区视觉基线曾因环境渲染漂移失败，已用 git stash 验证与代码改动无关，并于 R003 第一批收尾时重新生成基线；设置面板基线因阶段 6「本地存储」区于 R004 第三批更新。）常用命令：`npm run dev`、`npm run build`、`npm test`、`npm run typecheck`、`npm run lint`、`npm run deps:check`、`npm run ci`、`npm run test:e2e`（截图基线更新用 `npm run test:e2e:update`）。
